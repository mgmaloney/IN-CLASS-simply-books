...
const viewAuthorDetails = (authorFirebaseKey, user) => new Promise((resolve, reject) => {
  Promise.all([getSingleAuthor(authorFirebaseKey), getAuthorBooks(authorFirebaseKey, user)])
    .then(([authorObject, authorBooksArray]) => {
      resolve({ ...authorObject, books: authorBooksArray });
    }).catch((error) => reject(error));
});

const deleteAuthorBooks = (authorId, user) => new Promise((resolve, reject) => {
  getAuthorBooks(authorId, user).then((booksArray) => {
